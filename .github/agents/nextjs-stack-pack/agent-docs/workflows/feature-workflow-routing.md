...
This document defines Next.js-specific feature workflow routing deltas that overlay `agents-core` feature workflow routing.
...
| `accessibility.md` | When the diff changes interactive UI — user-visible markup, focus/keyboard behavior, dialogs/overlays, forms, or ARIA. Skip for pure logic/util/test-only diffs. |
| `react-composition.md` | When the diff adds or reshapes components, hooks, context, or a shared abstraction. Skip when no component/hook boundary moves. |
