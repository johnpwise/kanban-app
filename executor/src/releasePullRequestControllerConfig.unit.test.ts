...
describe("parseReleasePullRequestControllerConfig", () => {
  it("parses a valid environment", () => {
    const config = parseReleasePullRequestControllerConfig(validEnv);
  ...
  it.each(["ADA_RELEASE_INTENT_ID", "ADA_RELEASE_REPOSITORY"])("throws when %s is missing", (key) => {
    ...
    expect(() => parseReleasePullRequestControllerConfig(env)).toThrow();
  });
