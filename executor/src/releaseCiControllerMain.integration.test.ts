...
const entryPoint = path.join(__dirname, "..", "lib", "releaseCiControllerMain.js");
...
/**
 * Spawns the actual compiled release-CI-controller entry point as a real child process, mirroring
 * `releasePullRequestControllerMain.integration.test.ts`'s rationale exactly (real `process.exit`,
...
describe("releaseCiControllerMain.ts CLI entry point (compiled lib/releaseCiControllerMain.js)", () => {
  ...
  describeWithEmulator("with the Firestore emulator", () => {
    ...
    const app = initializeApp({ projectId }, `executor-release-ci-controller-cli-integration-${randomUUID()}`);
    getFirestore(app);
    ...
    // intent document exists either — this fails closed at the very first round's observation
    // (release intent load), for both targets, exactly the intended fail-closed behaviour. A
    // genuine `exit 0` requires a live GitHub App credential, a real repository, and durably
    // recorded release-PR results for both targets — deferred to the same explicit operational
    // approval boundary as the other controllers' own skipped end-to-end cases. This never touches
    // the real `release/0.1.1` intent or branch: the id below is a distinct, unrelated placeholder
    // version.
